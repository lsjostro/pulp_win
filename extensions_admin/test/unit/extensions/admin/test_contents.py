...
from pulp_win.extensions.admin import contents
...
class PackageSearchCommandTests(PulpClientTests):
    ...
    def test_run_search_with_field_filters(self, mock_search):
        ...
        # Test
        command = contents.BaseSearchCommand(self.context)
        command.run_search([ids.TYPE_ID_MSI], out_func=mock_out, **user_input)

        # Verify
        expected = {
            'type_ids': [ids.TYPE_ID_MSI],
            DisplayUnitAssociationsCommand.ASSOCIATION_FLAG.keyword: False,
        }
        mock_search.assert_called_once_with('repo-1', **expected)
        mock_out.assert_called_once_with(
            ['m'],
            contents.FIELDS_BY_TYPE[ids.TYPE_ID_MSI])


class SearchMsiCommand(PulpClientTests):
    def test_structure(self):
        command = contents.SearchMsiCommand(self.context)
        self.assertTrue(isinstance(command, contents.BaseSearchCommand))
        self.assertEqual(command.context, self.context)
        self.assertEqual(command.name, ids.TYPE_ID_MSI)
        self.assertEqual(command.description, contents.DESC_MSI)


class SearchMsmCommand(PulpClientTests):
    def test_structure(self):
        command = contents.SearchMsmCommand(self.context)
        self.assertTrue(isinstance(command, contents.BaseSearchCommand))
        self.assertEqual(command.context, self.context)
        self.assertEqual(command.name, ids.TYPE_ID_MSM)
        self.assertEqual(command.description, contents.DESC_MSM)
