...
from pulp_win.extensions.admin import copy_commands
from pulp_win.common.constants import CONFIG_RECURSIVE
...
class PackageCopyCommandTests(PulpClientTests):
    ...
    def test_get_formatter_for_type(self):
        command = copy_commands.MsmCopyCommand(self.context)

        # get a formatter and make sure it can be used
        formatter = command.get_formatter_for_type(TYPE_ID_MSM)
...
class OtherCopyCommandsTests(PulpClientTests):
    ...
    def test_msm_copy_command(self):
        # Test
        command = copy_commands.MsmCopyCommand(self.context)
        ...
        self.assertEqual(command.description, copy_commands.DESC_MSM)
